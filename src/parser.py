...
def p_classdecl(p):
	...
	p[0] = Class(p[2], p[3], p[4], p[5], token=p[1])
...
def p_classopts(p):
	...
	if len(p) == 2:
		...
		p[0] = ClassOpts(Type("Any"), Actuals(), token=p[1]) 
	if len(p) > 2:
		p[0] = ClassOpts(Native(token=p[2]), Actuals(), token=p[1])
	if len(p) > 3:
		p[0] = ClassOpts(p[2], p[3], token=p[1])

def p_type(p):
	"type : TYPE"
	p[0] = Type(p[1], token=p[1])

def p_varformals_empty(p):
	"varformals : '(' ')'"
	p[0] = Formals(token=p[1])

def p_varformals(p):
	"varformals : '(' var_list ')'"
	p[0] = Formals(p[2], token=p[1])

def p_var_list(p):
	"var_list : VAR id ':' type var_list_tail"
	p[0] = [Formal(p[2], p[4], token=p[1])] + p[5]
...
def p_var_init(p):
	...
	p[0] = VarInit(p[2], p[4], p[6], token=p[1])
...
def p_feature_var(p):
	...
	if len(p) == 3:
		...
		p[0] = VarInit(p[2], Native(token=p[4]), Native(token=p[4]), token=p[1])
...
def p_feature_def(p):
	...
	p[0] = Def(p[1], p[3], p[4], p[6], p[8], token=p[2])
...
def p_formal_list(p):
	...
	p[0] = [Formal(p[1], p[3], token=p[1])] + p[4]
...
def p_actuals_empty(p):
	"actuals : '(' ')' "
	p[0] = Actuals(token=p[1])

def p_actuals(p):
	"actuals : '(' exprlist ')'"
	p[0] = Actuals(p[2], token=p[1])
...
def p_primary_super(p):
	...
	p[0] = Super(p[3], p[4], token=p[1])

def p_primary_call(p):
	"primary : id actuals"
	p[0] = Call(p[1], p[2], token=p[1])

def p_primary_constructor(p):
	"primary : NEW type actuals"
	p[0] = Constructor(p[2], p[3], token=p[1])
...
def p_primary_int(p):
	"primary : INTEGER"
	p[0] = Integer(p[1], token=p[1])

def p_primary_bool(p):
	"primary : BOOLEAN"
	p[0] = Boolean(p[1], token=p[1])

def p_primary_str(p):
	"primary : STRING"
	p[0] = String(p[1], token=p[1])
...
def p_id(p):
	"id : ID"
	p[0] = Identifier(p[1], token=p[1])

def p_expr_assign(p):
	"expr : id '=' expr"
	p[0] = AssignExpr(p[1], p[3], token=p[2])
...
def p_control_if(p):
	...
	p[0] = IfExpr(p[3], p[5], p[7], token=p[1])

def p_control_while(p):
	"control : WHILE '(' expr ')' control"
	p[0] = WhileExpr(p[3], p[5], token=p[1])
...
def p_match(p):
	...
	p[0] = MatchExpr(p[1], p[3], token=p[2])
...
def p_case(p):
	...
	"""
	if len(p) == 7:
		p[0] = Case(p[2], p[4], p[6], token=p[1])
	else:
		# Intermediate: case null => E ==> case null:Null => E
		p[0] = Case(Null(token=p[2]), Type("Null", token=p[2]), p[4], token=p[1]) 
...
def p_comparison_lt(p):
	...
	p[0] = LTExpr(p[1], p[3], token=p[2])

def p_comparison_le(p):
	"comparison : comparison LE equiv"
	p[0] = LEExpr(p[1], p[3], token=p[2])
...
def p_sum_plus(p):
	"sum : sum '+' product"
	p[0] = AddExpr(p[1], p[3], token=p[2])

def p_sum_minus(p):
	"sum : sum '-' product"
	p[0] = SubExpr(p[1], p[3], token=p[2])
...
def p_product_times(p):
	...
	p[0] = MultExpr(p[1], p[3], token=p[2])

def p_product_div(p):
	"product : product '/' negation"
	p[0] = DivExpr(p[1], p[3], token=p[2])
...
def p_negation_not(p):
	"negation : '!' negation"
	p[0] = NotExpr(p[2], token=p[1])

def p_negation_neg(p):
	"negation : '-' negation"
	p[0] = NegExpr(p[2], token=p[1])
...
def p_dot_access(p):
	"dot : dot '.' id actuals"
	p[0] = Call(Dot(p[1], p[3], token=p[2]), p[4], token=p[2])
