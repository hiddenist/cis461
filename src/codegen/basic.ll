...
; since puts expects c-style strings, we'll use those (despite having the length argument)
...
define %obj_Boolean* @Any.equals(%obj_Any* %this, %obj_Any* %other) {
	...
	%bool = call %obj_Boolean* @Boolean._constructor(i1 %eq)
...
initialize:
	...
	; Store the string pointer in the appropriate field
	%strfield = getelementptr %obj_String* %obj, i32 0, i32 3
	store i8* %str, i8** %strfield
	
	; 1. Calculate the length of the string
	; 2. Create an Int of that length 
	; 3. Store the Int in the length field
